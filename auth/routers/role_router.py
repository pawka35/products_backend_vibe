...
@router.post("/users/assign", response_model=UserRole, status_code=status.HTTP_201_CREATED)
async def assign_role_to_user(
    ...
    check_user_permissions(user_role.user_id, current_user, db)
    
    try:
        return user_role_crud.assign_role_to_user(db, user_role, current_user.id)
...
@router.get("/users/{user_id}", response_model=List[UserRole])
async def get_user_roles(
    ...
    return user_role_crud.get_user_roles(db, user_id, active_only)
...
async def get_user_roles_detailed(
    ...
    return user_role_crud.get_user_roles_detailed(db, user_id)

@router.put("/users/{user_role_id}", response_model=UserRole)
async def update_user_role(
    ...
    current_user_role = user_role_crud.get_user_role(db, user_role_id)
    ...
    # Проверяем права на управление целевым пользователем
    check_user_permissions(current_user_role.user_id, current_user, db)
    
    # Если меняется роль, проверяем, не назначается ли уже эта роль
    if user_role_update.role_id and user_role_update.role_id != current_user_role.role_id:
        existing_role = user_role_crud.get_user_role(db, user_role_id)
        if existing_role:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    updated_user_role = user_role_crud.update_user_role(db, user_role_id, user_role_update)
    if not updated_user_role:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Связь пользователя с ролью не найдена."
        )
...
@router.delete("/users/{user_role_id}", status_code=status.HTTP_204_NO_CONTENT)
async def remove_role_from_user(
    ...
    if current_user_role.role.name == "admin":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Нельзя удалить системную роль 'admin' у пользователя."
        )
    
    success = user_role_crud.remove_role_from_user(db, user_role_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_users_by_role(
    ...
    return user_role_crud.get_users_by_role(db, role_id, skip, limit)
